...
export class Map extends maplibregl.Map {
  ...
  private usesLanguageFromStyle = true;
  ...
  setPrimaryLanguage(language: LanguageString = defaults.primaryLanguage) {
    if (language === Language.STYLE) {
      if (!this.usesLanguageFromStyle) {
        console.warn(
          "The language cannot be set back to the default style language.",
        );
      }
      return;
    }

    this.usesLanguageFromStyle = false;
